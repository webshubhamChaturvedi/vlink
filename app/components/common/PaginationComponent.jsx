...
export default function PaginationComponent({
  ...
  useEffect(() => {
    ...
    setPages((values) => {
      return { ...values, totalPages: Math.ceil(totalResults / limit) };
    });
  }, [totalResults, limit]);
  ...
  const calcPages = () => {
    ...
    buttons.push(
      <li
        ...
        className={`w-10 h-10 mx-1 inline-flex justify-center items-center text-[${
          page === 1 ? "#fff" : "#000"
        }] bg-[${
          page === 1 ? "#62207E" : "#CCCCCC"
        }] rounded-[100%] leading-[26px] cursor-pointer border border-[#eee] page-item ${
          sizes[1 - 1]
        }`}
      >
        <span
          disabled={page == 1}
          className={`page-link ${page === 1 ? "bg-light" : ""}`}
        >
          1
        </span>
      </li>
      ...
      for (let index = 2; index <= totalPages; index++) {
        buttons.push(
          ...
          >
            <span
              disabled={page == index}
              className={`page-link ${page == index ? "bg-light" : ""}`}
            >
              {index}
            </span>
          </li>
      ...
      {totalPages > 0 && (
        ...
        >
          <ul className="flex justify-end" style={{ minWidth: "max-content" }}>
            <li
              key={`page_${1}`}
              onClick={() => { if(page >1) backPage()}}
              className={`w-10 h-10 mx-1 inline-flex justify-center items-center text-[${ page > 1 ? "#fff" : "#000"}] bg-[${page > 1 ? "#62207E" : "#CCCCCC"}] rounded-[100%] leading-[26px] cursor-pointer border border-[#eee] page-item `}
            >
            ...
            <li
              ...
              onClick={() => nextPage()}
              className={`w-10 h-10 mx-1 inline-flex justify-center items-center text-[${
                page != totalPages ? "#fff" : "#000"
              }] bg-[${
                page != totalPages ? "#62207E" : "#CCCCCC"
              }] rounded-[100%] leading-[26px] cursor-pointer border border-[#eee] page-item `}
            >
              <span

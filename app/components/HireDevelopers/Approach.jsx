...
export default function Approach({ approach }) {
  ...
  return (
    <section
      className="pt-[55px] pb-[100px] md:pt-[35px] md:pb-[55px] bg-cover bg-no-repeat bg-[#F5FAFD]"
      // style={{
      //   backgroundImage: `url(${apiEndpoint(
      //     "/uploads/Rectangle_7099_73ece61c8f.png"
      //   )})`,
      // }}
    >
        ...
        <div className="approach_slider">
          <Slider {...settings}>
            {approach?.list?.map((items, key) => (
              <div
                key={key}
                className="bg-[#FFFFFF] p-[30px] rounded-[30px] shadow-[10px_10px_40px_0px_#0050D51F] transation-custom hover:shadow-[0px_0px_10px_#ccc] hover:scale-[1.01]"
              >
                <p className="text-[12px] font-[600] text-[#000D3B9E] mb-4">{key + 1} / {Object.keys(items).length - 1}</p>
                <div
                  className="p-1.5 rounded-[4px] inline-block mb-2"
